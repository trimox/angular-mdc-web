...
var MDC_PACKAGES = $MDC_ENTRYPOINTS_TMPL;
...
function setupLocalReleasePackages() {
  ...
  configureEntryPoint('packages');

  // Configure all secondary entry-points.
  MDC_PACKAGES.forEach(function(pkgName) {
    configureEntryPoint('web', pkgName);
  });
}
...
function configureEntryPoint(pkgName, entryPoint) {
  ...
  pathMapping['@angular-mdc/' + name] = packagesPath + '/' + name;
